...
export default function SubFamilias({familia}) {
  ...
  const { codigo, subfamilias} = familia;
	...
	const data = useMemo(() => subfamilias, []);

	return (
		<>
			<TemplateMaestroCodigos>
        <Link href="/mantenimiento/maestro-de-codigos/familias" className="flex gap-5 font-regular text-primary-500 hover:text-primary hover:font-bold">
          <Back/>
          <span>Familias</span>
        </Link>
...
export const getServerSideProps = async ({params}) => {
  ...
  const familia = familias.find((familia) => familia.codigo === params.idFamilia);
